/**
 * Exercise: 5-11
 * Modify  the  program  entab  and  entab  (written  as  exercises  in  Chapter  1)
...
void entab(char *inputLine, char *outputLine, int *tabStopPositions, int numTabStops);
int tabsAreIncreasing(int *tabStopPositions, int i);
int nextTabStopPosition(int column, int *tabStopPositions, int numTabStops);
...
int main(int argc, char *argv[])
{
    ...
    if ((numTabStops = getTabStopPositions(argc, argv, tabStopPositions)) < 0)
        ...
        {
            entab(inputLine, outputLine, tabStopPositions, numTabStops);
            printf("%s", outputLine);
        }
...
void entab(char *inputLine, char *outputLine, int *tabStopPositions, int numTabStops)
{
    ...
    for (i = j = 0; inputLine[i] != '\0'; i++)
        if (inputLine[i] == ' ')
        {
            ...
            {
                tabStopPos = nextTabStopPosition(i, tabStopPositions, numTabStops);
                if ((i + 1) == tabStopPos)
                {
                    numTabs++;
                    numBlanks = 0;
