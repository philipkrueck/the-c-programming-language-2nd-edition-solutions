...
void print_word_frequencies(struct tree_node *root);
void skip_blanks();
void skip_comments();
...
void print_word_frequencies(struct tree_node *parent)
{
    if (parent == NULL)
        return;

    print_word_frequencies(parent->left);
    if (parent->count > 0)
        printf("%s\n", parent->word);
    print_word_frequencies(parent->right);
}
